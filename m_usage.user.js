...
var usage={
"AbelianGroup" : "AbelianGroup[{n₁,n₂,…}] represents the direct product of the cyclic groups of degrees n₁,n₂,….", 
...
"AiryAiZero" : "AiryAiZero[k] represents the kᵗʰ zero of the Airy function Ai(x).\nAiryAiZero[k,x₀] represents the kᵗʰ zero less than x₀.", 
...
"AiryBiZero" : "AiryBiZero[k] represents the kᵗʰ zero of the Airy function Bi(x).\nAiryBiZero[k,x₀] represents the kᵗʰ zero less than x₀.", 
...
"ArrayFlatten" : "ArrayFlatten[{{m₁₁,m₁₂,…},{m₂₁,m₂₂,…},…}] creates a single flattened matrix from a matrix of matrices Subscript[m,ij]. \nArrayFlatten[a,r] flattens out r pairs of levels in the array a.", 
...
"BernsteinBasis" : "BernsteinBasis[d,n,x] represents the nᵗʰ Bernstein basis function of degree d at x.", 
...
"BesselJZero" : "BesselJZero[n,k] represents the kᵗʰ zero of the Bessel function Jₙ(x).\nBesselJZero[n,k,x₀] represents the kᵗʰ zero greater than x₀.", 
...
"BesselYZero" : "BesselYZero[n,k] represents the kᵗʰ zero of the Bessel function of the second kind Yₙ(x).\nBesselYZero[n,k,x₀] represents the kᵗʰ zero greater than x₀.", 
...
"BSplineBasis" : "BSplineBasis[d,x] gives the zeroth uniform B-spline basis function of degree d at x.\nBSplineBasis[d,n,x] gives the nᵗʰ uniform B-spline basis function of degree d.\nBSplineBasis[{d,{u₁,u₂,…}},n,x]  gives the nᵗʰ non-uniform B-spline basis function of degree d with knots at positions uᵢ.", 
...
"C" : "C[i] is the default form for the iᵗʰ parameter or constant generated in representing the results of various symbolic computations. ", 
...
"CatalanNumber" : "CatalanNumber[n] gives the nᵗʰ Catalan number Cₙ.", 
...
"CentralMoment" : "CentralMoment[list,r] gives the rᵗʰ central moment of the elements in list with respect to their mean.\nCentralMoment[dist,r] gives the rᵗʰ central moment of the symbolic distribution dist.\nCentralMoment[r] represents the rᵗʰ formal central moment. ", 
...
"Cumulant" : "Cumulant[dist,r] gives the rᵗʰ cumulant of the symbolic distribution dist.\nCumulant[list,r] gives the rᵗʰ cumulant of the elements in the list.\nCumulant[r] represents the rᵗʰ formal cumulant. ", 
...
"Cyclotomic" : "Cyclotomic[n,x] gives the nᵗʰ cyclotomic polynomial in x. ", 
...
"Default" : "Default[f] gives the default value for arguments of the function f obtained with a _. pattern object. \nDefault[f,i] gives the default value to use when _. appears as the iᵗʰ argument of f. \nDefault[f,i,n] gives the default value for the iᵗʰ argument out of a total of n arguments. \nDefault[f,…]=val defines default values for arguments of f.", 
...
"Differences" : "Differences[list] gives the successive differences of elements in list. \nDifferences[list,n] gives the nᵗʰ differences of list. \nDifferences[list,{n₁,n₂,…}] gives the successive nₖᵗʰ differences at level k in a nested list. ", 
"DifferentialD" : "DifferentialD[x] displays as x.", 
...
"E" : "E is the exponential constant  (base of natural logarithms), with numerical value ≃2.71828.", 
...
"Entropy" : "Entropy[list] gives the base  information entropy of the values in list.\nEntropy[k,list] gives the base k information entropy.", 
...
"Erfi" : "Erfi[z] gives the imaginary error function erf(iz)\/i. ", 
...
"ExponentialGeneratingFunction" : "ExponentialGeneratingFunction[expr,n,x] gives the exponential generating function in x for the sequence whose nᵗʰ term is given by the expression expr.\nExponentialGeneratingFunction[expr,{n₁,n₂,…},{x₁,x₂,…}] gives the multidimensional exponential generating function in x₁, x₂, … whose n₁, n₂, … term is given by expr.", 
...
"FactorialMoment" : "FactorialMoment[list,r] gives the rᵗʰ moment of the elements in the list.\nFactorialMoment[dist,r] gives the rᵗʰ moment of the symbolic distribution dist.\nFactorialMoment[r] represents the rᵗʰ factorial moment. ", 
...
"FindGeneratingFunction" : "FindGeneratingFunction[{a₁,a₂,…},x] attempts to find a simple generating function in x whose nᵗʰ series coefficient is aₙ.\nFindGeneratingFunction[{{n₁,a₁},{n₂,a₂},…},x] attempts to find a simple generating function whose nᵢᵗʰ series coefficient is aᵢ. ", 
...
"FlattenAt" : "FlattenAt[list,n] flattens out a sublist that appears as the nᵗʰ element of list. If n is negative, the position is counted from the end. \nFlattenAt[expr,{i,j,…}] flattens out the part of expr at position {i,j,…}. \nFlattenAt[expr,{{i₁,j₁,…},{i₂,j₂,…},…}] flattens out parts of expr at several positions. ", 
...
"FourierCoefficient" : "FourierCoefficient[expr,t,n] gives the nᵗʰ coefficient in the Fourier series expansion of expr.\nFourierCoefficient[expr,{t₁,t₂,…},{n₁,n₂,…}] gives a multidimensional Fourier coefficient.", 
"FourierCosCoefficient" : "FourierCosCoefficient[expr,t,n] gives the nᵗʰ coefficient in the Fourier cosine series expansion of expr.\nFourierCosCoefficient[expr,{t₁,t₂,…},{n₁,n₂,…}] gives a multidimensional Fourier cosine coefficient.", 
"FourierCosSeries" : "FourierCosSeries[expr,t,n] gives the nᵗʰ-order Fourier cosine series expansion of expr in t.\nFourierCosSeries[expr,{t₁,t₂,…},{n₁,n₂,…}] gives the multidimensional Fourier cosine series of expr.", 
...
"FourierSeries" : "FourierSeries[expr,t,n] gives the nᵗʰ-order Fourier series expansion of expr in t. \nFourierSeries[expr,{t₁,t₂,…},{n₁,n₂,…}] gives the multidimensional Fourier series.", 
"FourierSinCoefficient" : "FourierSinCoefficient[expr,t,n] gives the nᵗʰ coefficient in the Fourier sine series expansion of expr.\nFourierSinCoefficient[expr,{t₁,t₂,…},{n₁,n₂,…}] gives a multidimensional Fourier sine coefficient.", 
"FourierSinSeries" : "FourierSinSeries[expr,t,n] gives the nᵗʰ-order Fourier sine series expansion of expr in t.\nFourierSinSeries[expr,{t₁,t₂,…},{n₁,n₂,…}] gives the multidimensional Fourier sine series of expr.", 
...
"FourierTrigSeries" : "FourierTrigSeries[expr,t,n] gives the nᵗʰ-order Fourier trigonometric series expansion of expr in t.\nFourierTrigSeries[expr,{t₁,t₂,…},{n₁,n₂,…}] gives the multidimensional Fourier trigonometric series of expr.", 
...
"GeneratingFunction" : "GeneratingFunction[expr,n,x] gives the generating function in x for the sequence whose nᵗʰ series coefficient is given by the expression expr.\nGeneratingFunction[expr,{n₁,n₂,…},{x₁,x₂,…}] gives the multidimensional generating function in x₁, x₂, … whose n₁, n₂, … coefficient is given by expr.", 
...
"GoldenRatio" : "GoldenRatio is the golden ratio ϕ=(1)\/(2) (√(5)+1), with numerical value ≃1.61803.", 
...
"GraphPlot" : "GraphPlot[{Subscript[v,i1]->Subscript[v,j1],Subscript[v,i2]->Subscript[v,j2],…}] generates a plot of the graph in which vertex vᵢₖ is connected to vertex Subscript[v,jk].\nGraphPlot[{{Subscript[v,i1]->Subscript[v,j1],lbl₁},…}] associates labels lblₖ with edges in the graph.\nGraphPlot[m] generates a plot of the graph represented by the adjacency matrix m.", 
"GraphPlot3D" : "GraphPlot3D[{Subscript[v,i1]->Subscript[v,j1],Subscript[v,i2]->Subscript[v,j2],…}] generates a 3D plot of the graph in which vertex vᵢₖ is connected to vertex Subscript[v,jk].\nGraphPlot3D[{{Subscript[v,i1]->Subscript[v,j1],lbl₁},…}] associates labels lblₖ with edges in the graph.\nGraphPlot3D[m] generates a plot of the graph represented by the adjacency matrix m.", 
"GraphPower" : "GraphPower[g,n] gives the graph-nᵗʰ power of the graph g.", 
...
"HeavisidePi" : "HeavisidePi[x] represents the box distribution Π(x), equal to 1 for x<(1)\/(2) and 0 for x>(1)\/(2).\nHeavisidePi[x₁,x₂,…] represents the multidimensional box distribution Π(x₁,x₂,…) which is 1 if all xᵢ<(1)\/(2).", 
...
"HypergeometricPFQ" : "HypergeometricPFQ[{a₁,…,aₚ},{b₁,…,Subscript[b,q]},z] is the generalized hypergeometric function ₚSubscript[F,q](a;b;z). ", 
"HypergeometricPFQRegularized" : "HypergeometricPFQRegularized[{a₁,…,aₚ},{b₁,…,Subscript[b,q]},z] is the regularized generalized hypergeometric function ₚSubscript[F,q](a;b;z)\/(Γ(b₁)…Γ(Subscript[b,q])). ", 
...
"In" : "In[n] is a global object that is assigned to have a delayed value of the nᵗʰ input line. ", 
...
"Inherited" : "Inherited represents an option value to be inherited from an enclosing style, cell, or notebook. ", 
...
"InString" : "InString[n] is a global object that is assigned to be the text of the nᵗʰ input line. ", 
...
"LayeredGraphPlot" : "LayeredGraphPlot[{Subscript[v,i1]->Subscript[v,j1],Subscript[v,i2]->Subscript[v,j2],…}] generates a layered plot of the graph in which vertex vᵢₖ is connected to vertex Subscript[v,jk].\nLayeredGraphPlot[{{Subscript[v,i1]->Subscript[v,j1],lbl₁},…}] associates labels lblₖ with edges in the graph.\nLayeredGraphPlot[g,pos] places the dominant vertices in the plot at position pos.\nLayeredGraphPlot[m] generates a layered plot of the graph represented by the adjacency matrix m.", 
...
"LogitModelFit" : "LogitModelFit[{y₁,y₂,…},{f₁,f₂,…},x] constructs a binomial logistic regression model of the form 1\/(1+Superscript[, -(β₀+β₁f₁ +β₂f₂ +…)]) that fits the yᵢ for successive x values 1, 2, ….\nLogitModelFit[{{x₁₁,x₁₂,…,y₁},{x₂₁,x₂₂,…,y₂},…},{f₁,…},{x₁,x₂,…}] constructs a binomial logistic regression model of the form 1\/(1+Superscript[, -(β₀+β₁f₁ +β₂f₂ +…)]) where the fᵢ depend on the variables xₖ.\nLogitModelFit[{m,v}] constructs a binomial logistic regression model from the design matrix m and response vector v.", 
...
"MarginalDistribution" : "MarginalDistribution[dist,k] represents a univariate marginal distribution of the kᵗʰ coordinate from the multivariate distribution dist.\nMarginalDistribution[dist,{k₁,k₂,…}] represents a multivariate marginal distribution of the {k₁,k₂,…} coordinates.", 
...
"MatrixPower" : "MatrixPower[m,n] gives the nᵗʰ matrix power of the matrix m. \nMatrixPower[m,n,v] gives the nᵗʰ matrix power of the matrix m applied to the vector v.", 
...
"MessageList" : "MessageList[n] is a global object assigned to be a list of the names of messages generated during the processing of the nᵗʰ input line. ", 
...
"Moment" : "Moment[list,r] gives the rᵗʰ sample moment of the elements in list.\nMoment[dist,r] gives the rᵗʰ moment of the symbolic distribution dist.\nMoment[…,{r₁,r₂,…}] gives the {r₁,r₂,…}ᵗʰ multivariate moment. \nMoment[r] represents the rᵗʰ formal moment. ", 
...
"NBernoulliB" : "NBernoulliB[n] gives the numerical value of the nᵗʰ Bernoulli number. \nNBernoulliB[n, d] gives the result to d-digit precision.", 
...
"NIntegrate" : "NIntegrate[f,{x,xₘᵢₙ,xₘₐₓ}] gives a numerical approximation to the integral Subsuperscript[ ∫, xₘᵢₙ, xₘₐₓ] f dx. \nNIntegrate[f,{x,xₘᵢₙ,xₘₐₓ},{y,yₘᵢₙ,yₘₐₓ},…] gives a numerical approximation to the multiple integral Subsuperscript[ ∫, xₘᵢₙ, xₘₐₓ]dxSubsuperscript[ ∫, yₘᵢₙ, yₘₐₓ]dy … f.", 
...
"OrderDistribution" : "OrderDistribution[{dist,n},k] represents the kᵗʰ-order statistics distribution for n observations from the distribution dist.\nOrderDistribution[{dist,n},{k₁,k₂,…}] represents the joint (k₁,k₂,…)ᵗʰ-order statistics distribution from n observations from the distribution dist. ", 
...
"Out" : "%n or Out[n] is a global object that is assigned to be the value produced on the nᵗʰ output line. \n% gives the last result generated. \n%% gives the result before last. %%…% (k times) gives the kᵗʰ previous result. ", 
...
"PauliMatrix" : "PauliMatrix[k] gives the kᵗʰ Pauli spin matrix σₖ.", 
...
"PermutationPower" : "PermutationPower[perm,n] gives the nᵗʰ permutation power of the permutation perm.", 
...
"PolyGamma" : "PolyGamma[z] gives the digamma function ψ(z). \nPolyGamma[n,z] gives the nᵗʰ derivative of the digamma function Superscript[ψ, (n)](z). ", 
...
"PowerMod" : "PowerMod[a,b,m] gives aᵇ mod m. \nPowerMod[a,-1,m] finds the modular inverse of a modulo m.\nPowerMod[a,1\/r,m] finds a modular rᵗʰ root of a.", 
"PowerModList" : "PowerModList[a,s\/r,m] gives a list of all x modulo m for which xʳ≡aˢ mod m.", 
"PowersRepresentations" : "PowersRepresentations[n,k,p] gives the distinct representations of the integer n as a sum of k non-negative pᵗʰ integer powers.", 
...
"Prime" : "Prime[n] gives the nᵗʰ prime number. ", 
...
"ProbitModelFit" : "ProbitModelFit[{y₁,y₂,…},{f₁,f₂,…},x] constructs a binomial probit regression model of the form (1)\/(2) (1+erf((β₀+β₁f₁ +β₂f₂ +⋯)\/√(2))) that fits the yᵢ for successive x values 1, 2, ….\nProbitModelFit[{{x₁₁,x₁₂,…,y₁},{x₂₁,x₂₂,…,y₂},…},{f₁,f₂,…},{x₁,x₂,…}] constructs a binomial probit regression model of the form (1)\/(2) (1+erf((β₀+β₁f₁ +β₂f₂ +⋯)\/√(2))) where the fᵢ depend on the variables xₖ.\nProbitModelFit[{m,v}] constructs a binomial probit regression model from the design matrix m and response vector v.", 
...
"ProductLog" : "ProductLog[z] gives the principal solution for w in zweʷ. \nProductLog[k,z] gives the kᵗʰ solution. ", 
...
"QHypergeometricPFQ" : "QHypergeometricPFQ[{a₁,…,aᵣ},{b₁,…,bₛ},q,z] gives the basic hypergeometric series ᵣϕₛ(a;b;q;z).", 
...
"QPolyGamma" : "QPolyGamma[z,q] gives the q-digamma function Subscript[ψ,q](z).\nQPolyGamma[n,z,q] gives the nᵗʰ derivative of the q-digamma function Subsuperscript[ ψ, q, (n)](z).", 
...
"RankedMax" : "RankedMax[list,n] gives the nᵗʰ largest element in list.", 
"RankedMin" : "RankedMin[list,n] gives the nᵗʰ smallest element in list.", 
...
"Ratios" : "Ratios[list] gives the successive ratios of elements in list. \nRatios[list,n] gives the nᵗʰ iterated ratios of list. \nRatios[list,{n₁,n₂,…}] gives the successive nₖᵗʰ ratios at level k in a nested list. ", 
...
"SetInternetProxy" : "SetInternetProxy[\"protocol\", {\"proxyHost\", port}] causes Mathematica to use the specified proxy host and port when accessing URLs of the specified protocol (\"http\", for example). You can also use the Help > Internet Connectivity dialog in the front end to configure proxy settings.", 
"SetInternetProxy" : "SetInternetProxy[\"protocol\", {\"proxyHost\", port}] causes Mathematica to use the specified proxy host and port when accessing URLs of the specified protocol (\"http\", for example). You can also use the Help > Internet Connectivity dialog in the front end to configure proxy settings.", 
...
"SymmetricPolynomial" : "SymmetricPolynomial[k,{x₁,…,xₙ}] gives the kᵗʰ elementary symmetric polynomial in the variables x₁,…,xₙ.", 
...
"Transpose" : "Transpose[list] transposes the first two levels in list. \nTranspose[list,{n₁,n₂,…}] transposes list so that the kᵗʰ level in list is the nₖᵗʰ level in the result. ", 
...
"TreePlot" : "TreePlot[{Subscript[v,i1]->Subscript[v,j1],Subscript[v,i2]->Subscript[v,j2],…}] generates a tree plot of the graph in which vertex vᵢₖ is connected to vertex Subscript[v,jk].\nTreePlot[{{Subscript[v,i1]->Subscript[v,j1],lbl₁},…}] associates labels lblₖ with edges in the graph.\nTreePlot[g,pos] places roots of trees in the plot at position pos.\nTreePlot[g,pos,vₖ] uses vertex vₖ as the root node in the tree plot.", 
...
"Tuples" : "Tuples[list,n] generates a list of all possible n‐tuples of elements from list. \nTuples[{list₁,list₂,…}] generates a list of all possible tuples whose iᵗʰ element is from listᵢ. ", 
...
"UnitBox" : "UnitBox[x] represents the unit box function, equal to 1 for x≤(1)\/(2) and 0 otherwise.\nUnitBox[x₁,x₂,…] represents the multidimensional unit box function, equal to 1 if xᵢ≤(1)\/(2) and 0 otherwise.", 
...
"UnitVector" : "UnitVector[k] gives the two-dimensional unit vector in the kᵗʰ direction.\nUnitVector[n,k] gives the n-dimensional unit vector in the kᵗʰ direction.", 
...
"ZetaZero" : "ZetaZero[k] represents the kᵗʰ zero of the Riemann zeta function on the critical line.\nZetaZero[k,t] represents the kᵗʰ zero with imaginary part greater than t.", 
